    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m no-auto-tls 帮助(help)\e[0m: $(msg_ul $is_doc_url)\n"
...
get() {
    ...
    protocol)
        ...
        *socks*)
            net=socks
            is_protocol=socks
            [[ ! $is_socks_user ]] && is_socks_user=$is_alias
...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:"'${is_alias}-${net}-$is_addr'",add:"'$is_addr'",port:"'$port'",id:"'$uuid'",aid:"0",net:"'$net'",type:"'$header_type'",path:"'$kcp_seed'"}' <<<{})
    ...
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#${is_alias}-$net-${is_addr}"
    ...
    ws | h2 | grpc | xhttp)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            is_vmess_url=$(jq -c '{v:2,ps:"'${is_alias}-$net-$host'",add:"'$is_addr'",port:"'$is_https_port'",id:"'$uuid'",aid:"0",net:"'$net'",host:"'$host'",path:"'$path'",tls:"'tls'"}' <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        } || {
            [[ $is_trojan ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#${is_alias}-$net-$host"
        }
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#${is_alias}-$net-${is_addr}"
        ;;
    http)
        is_can_change=(0 1)
        is_info_show=(0 1 2)
    ...
    if [[ $is_no_auto_tls ]]; then
        ...
        msg "\e[41m帮助(help)\e[0m: $(msg_ul $is_doc_url)"
    fi
    footer_msg
}
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            msg
            msg "如果无法正常显示或识别, 请直接使用上面的 URL 自行生成二维码.\n"
            footer_msg
        }
    else
        [[ $1 == 'url' ]] && {
